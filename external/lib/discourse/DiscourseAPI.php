...
class DiscourseAPI
{
	...
	*/
	...
	function createPost($bodyText, $topicId, $categoryId, $userName, $title = false)
	{
		$params = array(
			'raw' => $bodyText,
			'archetype' => 'regular',
		...
		if(!$topicId) unset($params['topic_id']);
		if(!$title) unset($params['title']);
	...
	function getTopic($id) {
		...
		return $this->_getRequest("/t/{$id}.json");
	}

	function getTopicsFromCategory($category) {

		return $this->_getRequest("/c/{$category}.json");
	}

	function getTopicsPosts($topic_id) {

		return $this->_getRequest("/t/{$topic_id}/posts.json");
	}

	function getUserById($user_id) {

		return $this->_getRequest("/admin/users/{$user_id}.json");
	}
	...
	function likePost($post_id) {
		...
		return $this->_postRequest("/post_actions");
	}
}