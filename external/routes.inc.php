<?php
	...
	function route_file_upload() {
		...
		switch ($request->type) {
			...
			case 'post':
				...
				$data = array();
				$message = '';
				$upload = Upload::fromFile('attachment');
				if($upload -> isValid()){
					$attachment_Class = Attachments::upload($upload->asFile());
				}
			break;
		}
	...
	$site->getRouter()->addRoute('/files', 'route_file_upload', true);
?>