<?php
	...
	function recover_session() {
		...
		session_start();
		$random = new Random();
		$seed = ($site->user ? $site->user->id : '') . $random->token(16);
		$site->sid = isset($_SESSION['sid']) ? $_SESSION['sid'] : $site->hashToken($seed);
		$_SESSION['sid'] = $site->sid;
		return $site->sid;
	}

	function timestamp_expired($timestamp, $ttl, $time = null) {
		if (!$time) {
			$time = time();
		}
		return $timestamp < ($time - $ttl);
	}

	function percent_print($value, $decimals = 0) {
		if ($value < 1) {
			$value *= 100;
		}
